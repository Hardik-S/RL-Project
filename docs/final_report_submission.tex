...
A2C represents the ordinary actor-critic case: it estimates advantages and updates the policy with no explicit constraint on the size of the policy change beyond the optimizer and learning rate. PPO-Clip represents the most common practical PPO variant. Its clipped ratio objective limits the incentive to move the new policy too far from the old one on sampled actions, but it does not solve a constrained optimization problem exactly. PPO-KL represents a different PPO-family control mechanism: it keeps the same first-order training scaffold but stops policy epochs when the empirical KL becomes too large. TRPO represents the strongest trust-region version in the comparison because the KL constraint is built into the update itself. These differences make the comparison a direct test of increasing levels of policy-step control.
...
The evaluation was designed to avoid three common sources of misleading reinforcement-learning results. The first is selective reporting. Every planned run is represented in the manifest, and runs that collapsed are counted as collapsed rather than quietly excluded. This is why the final-return table states that collapsed runs remain in the mean at their last logged evaluation value. That choice is conservative: it prevents the benchmark from giving an algorithm credit only for the seeds that survived.

The second source of error is mixing training return and evaluation return. Training return can be affected by exploration noise, normalization state, and partial episodes collected during rollout. The reported scores instead use the periodic evaluation callback, with deterministic policy actions and a fixed evaluation cadence. This does not make the results noise-free, but it keeps the return definition consistent across algorithms and environments.

The third source of error is letting aggregate files become more authoritative than the raw run logs. During the final audit, some aggregate exports were found to be inconsistent with the per-run records. The report therefore treats \texttt{metrics.csv}, \texttt{updates.csv}, and \texttt{run\_status.json} as the source of truth. The consequence is that the report is more manual than ideal, but the numbers used in the final narrative are tied to run-level evidence rather than to stale summary files. For this project, that is the safer choice because the main claims depend on collapse counts and final logged evaluation checkpoints.
...
PPO-KL behaved differently from PPO-Clip in implementation terms: on the MuJoCo tasks, policy optimization stopped after only about \texttt{2.7--3.1} epochs on average rather than the nominal maximum of \texttt{10}. That shows the early-stop rule was active. It does not, by itself, prove that KL-based stopping caused the observed return differences rather than simply co-occurring with them, so the claim should remain narrow.
...
For a practitioner choosing among these methods under a limited compute budget, the results suggest three practical lessons. First, a plain actor-critic baseline is useful as a reference point, but it should not be treated as a reliable default for these continuous-control tasks. A2C was cheaper per run, but its lower cost did not compensate for poor HalfCheetah-v4 performance and three collapses. In a setting where failed training runs require manual inspection or reruns, the cheaper update can become more expensive in practice.

Second, PPO-Clip remains a strong engineering compromise. It avoids the complexity of TRPO, uses a first-order update, and performed much better than A2C on the main locomotion tasks. It also had only one collapse in the benchmark. If the goal were to recommend one simple baseline for a future student project, PPO-Clip would be the easiest method to justify because it is more stable than A2C here and much simpler to run than TRPO.

Third, the PPO-KL result shows why method names are not enough. PPO-KL was not just stock PPO with another label; it was implemented as an unclipped ratio surrogate with target-KL early stopping. The early-stop rule was active, but that did not guarantee the best robustness profile. On Walker2d-v4, PPO-KL had two collapses and a high standard deviation. The result is not that KL information is useless. Instead, it shows that the particular way KL is used matters. An early-stop fuse is weaker than solving an explicit trust-region update, and it may react only after the optimizer has already moved into a less useful region of policy space.
...
The submitted repository is \url{https://github.com/Hardik-S/RL-Project}. The intended repository root is \texttt{RL-Project}, not the broader workspace folder that contains drafts, lecture files, presentation materials, and other local artifacts. The repository contains the training code, configuration files, tests, documentation, and final report artifacts. The most relevant documentation files are \texttt{README.md}, \texttt{docs/REPRODUCIBILITY.md}, \texttt{docs/03\_experiment\_protocol.md}, \texttt{docs/04\_algorithm\_spec.md}, and \texttt{docs/final\_report\_submission.pdf}.

The reproducibility notes define the setup and commands needed to run tests, smoke checks, benchmark jobs, and analysis. The project also records manifests for planned and completed runs so that failed or collapsed jobs are visible rather than silently omitted. This matters for the main claim because excluding collapsed runs would make unstable algorithms look better than they were. In the final reported table, collapsed runs remain in the averages at their last logged evaluation return.
